...
| `9VialHolder-key.step` | A small "key" insert that locks the holder into place on the CubXL+ deck. |
...
3. Snap the assembled holder onto the CubXL+ deck.
...
- Deck: PANDA-BEAR **CubXL+ only** (does not fit the Cub deck)
...
```bash
python step_to_glb.py vial_holder
```
